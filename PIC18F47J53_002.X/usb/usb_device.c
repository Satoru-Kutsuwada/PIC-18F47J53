...
void USBDeviceTasks(void)
{
    ...
    {
        /*
         * USB モジュールを有効にした後、D+ または D- ラインの電圧が SE0 状態から
         * 抜け出すのに十分な高さまで上昇するまでに時間がかかります。 
         * USB リセット割り込みは、SE0 条件がクリアされるまでマスクを解除しないでください。 
         * これにより、ファームウェアがこの固有のイベントを USB ホストからの 
         * USB バス リセットとして誤って解釈するのを防ぐことができます。
         * 
    ...
    /*
     * タスク B: USB バス リセット割り込みのサービス。 サスペンド中にバス リセットが
     * 受信されると、ACTVIF が最初にセットされ、UCONbits.SUSPND がクリアされると、
     * 次に URSTIF ビットがアサートされます。 これが、ACTVIF の後に URSTIF が
     * チェックされる理由です。
     * 
     * USB リセット フラグは、USB 状態が DETACHED_STATE または ATTACHED_STATE の
     * 場合にはマスクされるため、これら 2 つの状態の間は USB リセット イベントを
     * 発生させることはできません。
     * 
    ...
    if(USBResetIF && USBResetIE)
    {
        ...
        /*
         * USBDeviceInit() 関数によって割り込みが無効になるため、割り込みを再度
         * 有効にします。 これはポーリング設定では何も行われません
         * 
    ...
    if(USBSOFIF)
    {
        ...
        USBClearInterruptFlag(USBSOFIFReg,USBSOFIFBitNum);
            ...
            /*
             * ステータスステージを準備せずに、制御転送の処理が進んでから時間が経過し
             * すぎていないか確認してください。 その場合は、ステータス ステージを
             * 自動準備して、公式 USB 2.0 仕様のセクション 9.2.6 で規定されている
             * タイミング制限内で制御転送が [最終的に] 完了できるようにします。
            ...
            {
                USBCtrlEPAllowStatusStage();    //ステータスステージがすでに装備されている場合は何もしません。Does nothing if the status stage was already armed.
            }
        #endif
    }
    ...
    if(USBErrorIF && USBErrorIE)
    {
        ...
        /*
         * PIC18 では、エラーの原因をクリアすると、割り込みフラグが自動的にクリア
         * されます。 他のデバイスでは、割り込みフラグを手動でクリアする必要があります。
         * 
    ...
    /*
     * ホストがバス リセットを送信していない場合、サービスを続行しても意味がありません。 
     * バス リセットが受信されると、デバイスは DEFAULT 状態に移行し、通信の準備が整います。
     * 
    ...
    /*
     * タスク D: USB トランザクション完了割り込みの処理
     * Task D: Servicing USB Transaction Complete Interrupt
     */
    if(USBTransactionCompleteIE)
    {
        /*
         * USAT FIFO エントリを空にするか空にします。 USB FIFO がいっぱいになると、
         * USB 帯域幅の使用率が低下する可能性があり、デバイスは SETUP パケットを受信できなくなります。
         * 
         * Drain or deplete the USAT FIFO entries.  If the USB FIFO ever gets 
         * full, USB bandwidth utilization can be compromised, and the device 
         * won't be able to receive SETUP packets.
         */
        for(i = 0; i < 4u; i++){
            if(USBTransactionCompleteIF){
                /*
                 * USTAT レジスタ情報を保存および抽出します。 この情報は後で使用します。
                ...
                /*
                 * ピンポン バッファリングが有効になっている場合は、EP0 以外の
                 * エンドポイントのハードウェア ピンポン状態を追跡します。
                 * 
                 * Keep track of the hardware ping pong state for endpoints 
                 * other than EP0, if ping pong buffering is enabled.
                 */
                ...
                /*
                 * USBCtrlEPService は EP0 を介したトランザクションのみを処理します。 
                 * 他のすべての EP トランザクションは無視されます。
...
/**************************************************************************
    ...
    Summary:
        ...
        the USB host.
 * この関数は、USB モジュール自体を USB ホストから「ソフト デタッチ」するように設定します。
 * 
 ...
 * 
 * この機能は、D+ (または D-) ~1.5k プルアップ抵抗を無効にすることで「ソフト デタッチ」操作を
 * 実行するように USB モジュールを構成します。
 * これにより、デバイスが存在し接続されていることをホストに知らせます。 
 * これにより、ホストはデバイスが接続されていないと認識します。 
 * これは、USB デバイスがホストにデバイスの再列挙を強制できるようにするため、潜在的に役立ちます
 *  (ファームウェアでは、USBDeviceAttach() を呼び出して USB モジュール/プルアップを再有効化し
 * 、「ソフト再アタッチ」します) ザ・ホスト）。
 * 
 ...
 * 
 * USB_INTERRUPT が定義されている場合にのみ呼び出す必要があります。 USB_POLLING モード 
 * オプション (usb_config.h オプション) が使用されている場合は、備考セクションを参照してください。
 * 
 * さらに、この関数は main() ループ コンテキストからのみ呼び出す必要があります。
 * この関数はグローバル割り込みイネーブルビットと設定を変更する可能性があるため、
 ...
 * 
 *      この関数は、USB デバイスがバスに接続されていることを USB ホストに示します。
 *      デバイスがバス上で列挙を開始するには、この関数を呼び出す必要があります。
 * 
 * Description:
 ...
 * 
 *      USB_INTERRUPT が定義されている場合にのみ呼び出す必要があります。 
 *      また、main() ループ コンテキストからのみ呼び出す必要があります。 
 *      USBDeviceAttach() 関数はグローバル割り込みイネーブル ビットと設定を変更する
 *      可能性があるため、割り込みハンドラ内から USBDeviceAttach() を呼び出さないでください。
 * 
 ...
 * 
 * モジュールが以前にオンだった場合は、この関数を呼び出してモジュールを再度有効にする前に、
 * 長期間 (例: 100ms 以上) オフになっていることを確認してください。 
 * デバイスが D+ (フルスピードの場合) または D- (低速の場合) ～ 1.5k オームの
 * プルアップ抵抗をオフにし、その後すぐにオンに戻すと、人間には不可能なため、
 * 一般的なホストがこのイベントを拒否することがあります。 USB デバイスの取り外しと
 * 再接続をマイクロ秒 (またはナノ秒) の時間スケールで実行します。 ホストはこれを
 * ある種の不具合として単純に扱い、イベントを完全に無視することができます。
 * 
 *  Parameters: None
 * 
 *  Return Values:None
