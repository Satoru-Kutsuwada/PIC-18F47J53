...
/*
 * <USB_HANDLE>
 * USB_HANDLE は、BDT 内のエントリへのポインタです。 このポインタを使用して、最後の
 * 転送の長さ、最後の転送のステータス、およびその他のさまざまな情報を読み取ることが
 * できます。 
 * USB_HANDLE オブジェクトを必ず NULL に初期化して、最初の使用時に既知の状態になる
 * ようにしてください。
 * 
 ...
 * and various other information.  Insure to initialize USB_HANDLE objects to 
 * NULL so that they are in a known state during their first usage.
 * 
 */
...
{
    /* 
     * デタッチとは、デバイスがバスに接続されていない状態です。 
     * 切り離された状態のとき、デバイスは D+ または D- ラインにプルアップを接続してはなりません。
     * 
     * Detached is the state in which the device is not attached to the bus.  
     * When in the detached state a device should not have any pull-ups attached 
     * to either the D+ or D- line.
     * 
     */
    DETACHED_STATE
    ...
    /*
     * 接続済みとは、デバイスがバスに接続されていますが、接続先のハブ/ポートがまだ構成されていない状態です。
    ...
    /* 
     * 電源が入っている状態は、デバイスがバスに接続されており、接続されているハブ/ポートが構成されている状態です。
    ...
    /* 
     * アドレス保留状態は、USB で定義された状態の正式な状態ではありません。 
     * この状態は、デバイスが SET_ADDRESS コマンドを受信したが、転送の STATUS 
     * ステージをまだ受信していないことを示すために内部的に使用されます。 
     * デバイスは、STATUS ステージが完了するまでアドレスを切り替えないでください。
     *
     * Address pending state is not an official state of the USB defined states. 
     * This state is internally used to indicate that the device has received 
     * a SET_ADDRESS command but has not received the STATUS stage of the 
     * transfer yet. The device is should not switch addresses until after 
     * the STATUS stage is complete.
     */
    ADR_PENDING_STATE
    /*DOM-IGNORE-BEGIN*/    = 0x08                         /*DOM-IGNORE-END*/,

    /* アドレスは、デバイスがバス上で独自の特定のアドレスを持っている状態です。
     * Address is the state in which the device has its own specific address 
     * on the bus. 
     */
    ADDRESS_STATE
     ...
     * これで、デバイスはアプリケーション固有のタスクを実行できるようになります。 
     * また、現在の構成の構成記述子で指定された値まで電流消費を増やすこともできます。
     * Configured is the state where the device has been fully enumerated and 
     * is operating on the bus.  The device is now allowed to execute its
     *  application specific tasks.  It is also allowed to increase its current
     *  consumption to the value specified in the configuration descriptor of 
